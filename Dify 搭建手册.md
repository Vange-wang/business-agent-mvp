...
当前 Dify Cloud 已完成的部分：

- 已登录 Dify Cloud。
- 已上传知识库文件 `knowledge-base.md`。
- 已因未配置 Embedding 模型，将知识库索引方式切换为 Economy / 经济模式。
- 已创建 Chatflow 应用：`智能客服业务智能体 MVP`。
- 已在画布中保留默认主线：Start / 用户输入 / LLM / 直接回复。
- 已新增直接回复节点：
  - `转人工回复`
  - `线索收集回复`
  - `未知兜底回复`
...
1. Dify → Settings → Model Provider，添加模型供应商 API Key。
2. 回到 `智能客服业务智能体 MVP` Chatflow。
3. 打开 LLM 节点，选择模型。
4. 将 `Agent研究文档/Dify模板/智能客服业务智能体/system-prompt.md` 粘贴进系统提示词。
5. 添加 Knowledge Retrieval 节点，关联 `knowledge-base.md` 对应知识库。
6. 添加条件分支或问题分类器，路由到：
   - 知识库问答
   - 线索收集回复
   - 转人工回复
   - 未知兜底回复
7. 用 `test-cases.md` 完成至少 3 条冒烟测试。
8. 测试通过后发布应用。
...
- Dify Cloud 前端在 LLM 上下文面板出现过“渲染此组件时发生了意外错误”。
- 已通过 Dify 同源 Console API 读取并修复 draft workflow。
- 修复内容：
  - 开启 LLM `context.enabled`。
  - 将 LLM context 绑定到 Knowledge Retrieval 节点的 `result`。
  - 写入 `system-prompt.md` 系统提示词。
  - 将边调整为 `Start → Knowledge Retrieval → LLM → Answer`。
- 修复后回到 Dify UI 预览面板完成测试，并发布成功。
...
| 你们怎么收费？多少钱？ | 通过。未给固定最终报价，要求根据场景评估。 |
| 我要转人工，找真人客服 | 通过。识别转人工意图并引导人工客服跟进。 |

当前剩余增强：

- 可继续补条件分支，将转人工、线索收集、报价咨询、未知兜底拆成独立分支。
- 可将知识库索引从 Economy 升级为 High Quality。
- 可在 Access API 中创建应用调用凭证，接入本地演示页或客户网站。
